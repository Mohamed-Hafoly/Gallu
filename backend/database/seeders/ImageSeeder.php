...
class ImageSeeder extends Seeder
{
    ...
    /**
     * Where real photographs may be dropped to seed a gallery worth looking at.
     *
     * Optional and gitignored: `database/seed-images/*.{jpg,jpeg,png,webp}` is
     * used when it holds anything, and the generated colour blocks below are the
     * fallback when it does not. That keeps the seed dependency-free for a fresh
     * clone while letting a local run produce screenshots of something other
     * than a grid of flat colours.
     */
    private const PHOTO_DIRECTORY = 'seed-images';

    /**
     * Absolute paths of the pool, or null before the first seedInto() call.
     *
    ...
    /**
     * Whether this pool was generated by us, and so is ours to delete. Photos
     * supplied in PHOTO_DIRECTORY belong to whoever put them there.
     */
    private bool $poolIsDisposable = true;
    ...
    public function seedInto(Document $document, User $owner, int $count = 4): void
    {
        ...
        $this->placeholders ??= $this->resolvePool();
        ...
        Image::factory()
            ...
            ->each(function (Image $image): void {
                $source = $this->placeholders[array_rand($this->placeholders)];

                $image->addMedia($source)
                    // preservingOriginal(), or the first image to use a given
                    // source would move it off disk and leave every later one
                    // adding a file that is no longer there.
                    ->preservingOriginal()
                    ->usingName($image->title)
                    // The source's own extension, not a hardcoded .jpg: a photo
                    // pool may hold png or webp, and a name that disagrees with
                    // the bytes misleads every consumer of the media row.
                    ->usingFileName(Str::uuid().'.'.pathinfo($source, PATHINFO_EXTENSION))
    ...
    /**
     * The photographs in PHOTO_DIRECTORY if there are any, generated colour
     * blocks otherwise.
     *
     * @return list<string>
     */
    private function resolvePool(): array
    {
        $photos = glob(database_path(self::PHOTO_DIRECTORY).'/*.{jpg,jpeg,png,webp}', GLOB_BRACE) ?: [];

        if ($photos === []) {
            return $this->generatePlaceholders();
        }

        // Not ours to delete once the run is over - unlike the generated pool,
        // these are files somebody put there on purpose. addMedia() would move
        // them away too, which preservingOriginal() in seedInto() prevents.
        $this->poolIsDisposable = false;
